...
const createWorkspaceService = async (userId, body) => {
    ...
    const existingWorkspace = await workspace_model_1.default.findOne({
        name: name,
        owner: userId,
    });
    if (existingWorkspace) {
        throw new appError_1.BadRequestException("You already have a workspace with this name. Please choose a different name.");
    }
    if (!user) {
